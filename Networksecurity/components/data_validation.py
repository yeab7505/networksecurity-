...
class DataValidation: 
        ...
        try:
            ...
            for col in numerical_columns:
                if col not in dataframe_columns:
                    logging.info(f"Column: [{col}] is not present in the dataframe")
                    return False
            return True
        except Exception as e:
            raise NetworkSecurityException(e, sys)
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message=("Train data does not have the required number of columns")
                raise Exception(error_message)
            status=self.validation_number_of_columns(test_dataframe)
            if not status:
                error_message=("Test data does not have the required number of columns")
                raise Exception(error_message)
            status=self.validate_numerical_columns(train_dataframe)
            if not status:
                error_message=("Train data does not have all the required numerical columns")
                raise Exception(error_message)
            status=self.validate_numerical_columns(test_dataframe)
            if not status:
                error_message=("Test data does not have all the required numerical columns")
                raise Exception(error_message)
